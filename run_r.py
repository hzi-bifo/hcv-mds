...
def findmin(hi):  # find minimum across each column
    ...
    for i in range(len(hi[0])):
        for j in range(len(hi)):
            #print (hi[j][i])
            if str(hi[j][i]).strip() == "*" or str(hi[j][i]
                                                   ).strip() == "<" or str(hi[j][i]).strip() == ">":
                continue
            else:
                # print max
...
def addoptions():
    parser = argparse.ArgumentParser(
        description="Take neutralization matrix as input and give the final antigenic plot",
        formatter_class=argparse.RawTextHelpFormatter,
        usage="%(prog)s inputfile [-h] [-norm NORMALIZE] [-w WEIGHT] [-dim DIM]")
