...
def gap_statistic(X, refs=None, B=10, K=range(1, 11), N_init=10):
    ...
    for indk, k in enumerate(K):
        ...
        k_means = Kmeans(n_clusters=k, init='k-means++', n_init=N_init, max_iter=300, tol=0.0001,
...
def find_optimal_k(gaps, s_k, K):
    ...
    printLog(
        'here is the comparsion between gap_k and (gap_k+1 - s_k+1):', below_or_above)
    if below_or_above.any():
        optimal_k = K[below_or_above.argmax()]
    else:
        printLog(
            'have NOT found the best k above the next k+1 threshold yet, use the last k instead')
        optimal_k = K[-1]
    return optimal_k
